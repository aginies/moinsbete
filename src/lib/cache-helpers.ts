...
export async function cleanupExpired() {
  const now = new Date()
  const [cnrs, radio, wiki, wikiLoves, news, f1, portailWikipedia] = await Promise.all([
  ...
  return { cnrs: cnrs.count, radio: radio.count, wiki: wiki.count, wikiLoves: wikiLoves.count, news: news.count, f1: f1.count, portailWikipedia: portailWikipedia.count }
}
