...
export async function cleanupExpired() {
  const now = new Date()
  const [cnrs, radio, wiki, wikiLoves, news, f1, portailWikipedia, citation, insolite, apod, airCrash] = await Promise.all([
    ...
    prisma.cachedAirCrashArticle.deleteMany({ where: { expiresAt: { lt: now } } }),
  ])

  return { cnrs: cnrs.count, radio: radio.count, wiki: wiki.count, wikiLoves: wikiLoves.count, news: news.count, f1: f1.count, portailWikipedia: portailWikipedia.count, citation: citation.count, insolite: insolite.count, apod: apod.count, airCrash: airCrash.count }
}
