...
import { extractEntriesFR, extractEntriesEN, type WikipediaImageEntry } from '@/lib/wikipedia-image-parse'
...
interface WikipediaImageConfig {
  ...
  configKey: string
  filePrefix: string
  extractEntries: (html: string, archive: string) => WikipediaImageEntry[]
  logScraping: string
  logNoData: string
  logUpserted: string
}
...
async function fetchArchive(config: WikipediaImageConfig, archiveName: string): Promise<WikipediaImageEntry[] | null> {
  try {
