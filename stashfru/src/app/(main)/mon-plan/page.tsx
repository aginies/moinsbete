...
export default async function MonPlanPage() {
  ...
  if (planIdeas.length === 0 && topicIds.length > 0) {
    const topicsRes = await fetch(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/feed?topic=${topicIds[0]}&page=1&limit=10`,
      { next: { revalidate: 60 } }
    )
  ...
  if (planIdeas.length === 0) {
    const feedRes = await fetch(
      `${process.env.NEXTAUTH_URL || 'http://localhost:3000'}/api/feed?page=1&limit=10`,
      { next: { revalidate: 60 } }
    )
